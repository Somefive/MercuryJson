...
namespace MercuryJson {
    ...
    private:
        ...
        inline void check_alloc(size_t size) {
            if (allocated + size > block_size) {
                allocated = 0;
                ptr = reinterpret_cast<char *>(aligned_malloc(block_size, alignment));
                if (parent != nullptr) parent->all_memory.push_back(ptr);
    ...
    public:
        ...
        BlockAllocator fork() { return {block_size, this}; }

        BlockAllocator fork(size_t block_size) { return {block_size, this}; }
