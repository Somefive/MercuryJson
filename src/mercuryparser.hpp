...
namespace MercuryJson {
    ...
    u_int64_t extract_escape_mask(const Warp & raw, u_int64_t & prev_odd_backslash_ending_mask) {
        ...
        u_int64_t start_backslash_mask          = backslash_mask & ~(backslash_mask << 1);
        ...
        u_int64_t odd_backslash_ending_mask     = (odd_carrier_backslash_mask < odd_start_backslash_mask) | prev_odd_backslash_ending_mask;
    ...
    u_int64_t extract_literal_mask(const Warp & raw, u_int64_t escape_mask, u_int64_t & prev_literal_ending, u_int64_t & quote_mask) {
        quote_mask                              = __cmpeq_mask(raw, '"') & ~escape_mask;
        u_int64_t literal_mask                  = _mm_cvtsi128_si64(_mm_clmulepi64_si128(_mm_set_epi64x(0ULL, quote_mask), _mm_set1_epi8(0xFF), 0));
        u_int64_t literal_reversor              = prev_literal_ending * ~0ULL;
        literal_mask                            ^= literal_reversor;
        prev_literal_ending                     = (literal_mask & (1ULL << 63)) >> 63;
        return literal_mask;
    }

    void extract_structural_whitespace_characters(const Warp & raw, u_int64_t literal_mask, u_int64_t & structural_mask, u_int64_t & whitespace_mask) {
        const __m256i upper_lookup = _mm256_setr_epi8(8, 0, 17, 2, 0, 4, 0, 4, 0, 0, 0, 0, 0, 0, 0, 0, 8, 0, 17, 2, 0, 4, 0, 4, 0, 0, 0, 0, 0, 0, 0, 0);
        const __m256i lower_lookup = _mm256_setr_epi8(16, 0, 0, 0, 0, 0, 0, 0, 0, 8, 10, 4, 1, 12, 0, 0, 16, 0, 0, 0, 0, 0, 0, 0, 0, 8, 10, 4, 1, 12, 0, 0);
        __m256i hi_upper_index      = _mm256_shuffle_epi8(upper_lookup, _mm256_and_si256(_mm256_srli_epi16(raw.hi, 4), _mm256_set1_epi8(0x7F)));
        ...
        __m256i lo_upper_index      = _mm256_shuffle_epi8(upper_lookup, _mm256_and_si256(_mm256_srli_epi16(raw.lo, 4), _mm256_set1_epi8(0x7F)));
    ...
    u_int64_t extract_pseudo_structural_mask(u_int64_t structural_mask, u_int64_t whitespace_mask, u_int64_t quote_mask, u_int64_t literal_mask, u_int64_t & prev_pseudo_structural_end_mask) {
        u_int64_t st_ws                     = structural_mask | whitespace_mask;
        structural_mask                     |= quote_mask;
        u_int64_t pseudo_structural_mask    = (st_ws << 1) | prev_pseudo_structural_end_mask;
        prev_pseudo_structural_end_mask     = (st_ws >> 63) & 1ULL;
    ...
    void construct_structural_character_pointers(u_int64_t pseudo_structural_mask, size_t offset, size_t * indices, size_t & base) {
